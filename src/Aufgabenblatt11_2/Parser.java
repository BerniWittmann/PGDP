...
public class Parser {
  ...
  private Declaration parseDecl() {
    ...
    if (tokens[from].equals(";")) {
      ...
      return new Declaration((String[]) variables.toArray(), ind);
    } else
      return null;
  }
  ...
  private Expression parseExpressionNoBinop() {
    ...
    String name = parseName();
    if (name != null) {
      return new Variable(name, parseIndex());
    }
    // (<expr>)
    from = before;
    if (tokens[from].equals("(")) {
  ...
  private Statement parseAssigment() {
    ...
    if (exp == null)
      return null;
    return new Assignment(variableName, exp, index);
  }
