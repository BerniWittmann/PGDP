...
import static Aufgabenblatt11_2.SteckOperation.ADD;
import static Aufgabenblatt11_2.SteckOperation.NOP;
    ...
    private int executeHelp(int[] program) {
        ...
        while (true) {
            ...
            switch (insn.getOperation()) {
                ...
                //** Also der Heap soll ja auf den Stack geladen werden bin mir jetzt unsicher ob das so passt??
                case LDH: {
                    int stackAddress = framePointer + parameter + pop();
                    if (stackAddress < 0 || stackAddress >= stack.length)
                        error("Invalid stack access");
                    push(stack[stackAddress]);
                    break;
                ...
                //** Ich weiß ehrlich gesagt nicht wie ich das
                case STH: {
                    int stackAddress = framePointer + parameter + pop();
                    if (stackAddress < 0 || stackAddress >= stack.length)
                        error("Invalid stack access");
                ...
                //** Allokiert die Werte und gibt die Stack-Werte auf den Heap zurücl
                case ALLOCH: {
                    int a = pop();
                    int b = pop();
                    int stackAddress = framePointer + parameter + a + b;
                    if (stackAddress < 0 || stackAddress >= stack.length)
                        error("Invalid stack access");
                    int value = pop();
                    stack[stackAddress] = value;
